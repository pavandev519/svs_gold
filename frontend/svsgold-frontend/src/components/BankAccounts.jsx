      ...
      {isOpen && (
        <div className="accordion-content space-y-6 px-6 py-6 bg-gradient-to-b from-white/50 to-amber-50/30 border-t border-white/50">
          {bankAccounts.length === 0 ? (
            <button
              onClick={addNewBank}
              className="w-full py-3 border-2 border-dashed border-amber-500 text-amber-700 font-semibold rounded-xl hover:bg-amber-50 transition-all duration-300 flex items-center justify-center gap-2"
            >
              <Plus size={20} />
            ...
            bankAccounts.map((bank, index) => (
              <div key={bank.id} className="bg-white/80 backdrop-blur-sm p-6 rounded-xl border-2 border-amber-100/50 space-y-4 hover:shadow-md transition-all duration-300">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="font-semibold text-gray-800">
                      Bank Account {index + 1}
                      {bank.is_primary && <span className="ml-3 text-xs bg-gradient-to-r from-amber-500 to-amber-700 text-white px-3 py-1 rounded-full">Primary</span>}
                    </h3>
                  </div>
                  <button
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-gradient-to-b from-white to-gray-50 border-2 border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:border-amber-600 focus:ring-4 focus:ring-amber-600/10 transition-all duration-300 shadow-sm hover:shadow-md hover:border-gray-300"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 bg-gradient-to-b from-white to-gray-50 border-2 border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:border-amber-600 focus:ring-4 focus:ring-amber-600/10 transition-all duration-300 shadow-sm hover:shadow-md hover:border-gray-300"
                    >
